...
if [ "$auth_choice" = "1" ]; then
    ...
    echo "   • Upload Certificate: $(pwd)/certs/certificate.crt"
    echo ""
    echo "5. OAuth Scopes:"
    echo "   • Access and manage your data (api)"
    echo "   • Perform requests on your behalf at any time (refresh_token, offline_access)"
    echo ""
    echo "6. Save → Wait 2-10 min → Manage → Edit Policies"
    echo "   • Permitted Users: 'Admin approved users are pre-authorized'"
    echo ""
    echo "7. Manage Profiles/Permission Sets:"
    echo "   • Add your integration user's profile"
    echo ""
